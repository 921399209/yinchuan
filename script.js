...
4. Default replacement logic: this is face/identity replacement, not full-body redesign. The new image should use the user's selfie identity/face while keeping the reference character's facial expression, gaze direction, head angle, body pose, hands, legs, outfit, hairstyle silhouette, accessories, lighting, camera angle, composition, and environment consistent with the effect image.
...
9. For the selected subject in “你的照片主体”, do not describe the old facial identity, exact facial features, old skin tone, old face shape, or old personal identity. Instead write that the face/identity should come from the uploaded selfie/photo, while the facial expression, eye direction, head angle, pose, clothing, outfit colors, accessories, hair styling seen in the effect, body posture, and scene must remain the same as the effect image.
10. Lock the effect image's subject styling unless the user explicitly chooses a clothing-replacement strategy: clothing type, clothing color, fabric, shoes, accessories, hairstyle shape, eye gaze, expression, and body action should be described as fixed visual targets. If hands, arms, legs, or body shape are visible, describe their position and gesture accurately.
11. Everything outside the selected subject's identity must be locked in detail. If there is an AI-generated companion/person, describe their age range, ethnicity or regional look, skin tone, facial vibe, hair color, hairstyle, body type, outfit, shoes, bag, accessories, expression, pose, position, gaze, and interaction.
12. If visible text, plaques, lanterns, signs, usernames, screens, posters, storefronts, or interface elements appear, describe their position, color, material, typography style, and readable or approximate text. Do not omit sign text.
13. If the image has a raw viral prompt style, infer that style but keep the final Main Prompt precise and usable.
14. Negative Prompt must be practical and image-specific, including wrong face identity, changed expression, wrong gaze direction, wrong pose, changed clothing, wrong camera angle, wrong lighting direction, missing background details, unreadable text, changed outfit on non-replaceable characters, and missing visible props when relevant.
15. Suggested Model / Style should name suitable model families or generation modes, for example GPT image generation, Midjourney realistic/editorial, Flux photoreal, cinematic composite, fashion editorial, or product render, based on the image.
16. Suggested Parameters should include useful settings such as aspect ratio, lens/camera feel, quality/detail, stylize/style strength, CFG, steps, seed consistency, and any image-reference strength guidance. Use generic parameters when platform-specific settings are uncertain.
17. Return JSON only. No Markdown, no numbered list text, no code block.
